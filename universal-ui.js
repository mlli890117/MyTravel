/* My Travel universal form editors v23 */
(() => {
  ...
  function ensureAddAddress(){
    const note=$('iNote');if(!note||$('iAddress'))return;
    const input=document.createElement('input');input.id='iAddress';input.className='wide';input.placeholder='地址／地點名稱（可自動定位，例如：新千歲機場、Watermark Hotel Miyakojima）';
    note.after(input);
  }
  ...
  window.addItem=async function(){
    ensureAddAddress();
    const title=$('iTitle')?.value.trim();if(!title)return alert('請輸入名稱');
    const address=$('iAddress')?.value.trim()||'';
    let lat=parseFloat($('iLat')?.value),lng=parseFloat($('iLng')?.value),found=null;
    if(address)found=await geocodeAddress(address);
    if(found){lat=found.lat;lng=found.lng;$('iLat').value=lat;$('iLng').value=lng}
    const t=trip();t.itinerary[curDate].push({id:crypto.randomUUID(),time:$('iTime').value,title,type:$('iType').value,note:$('iNote').value.trim(),address,lat:Number.isFinite(lat)?lat:null,lng:Number.isFinite(lng)?lng:null});
    ['iTitle','iNote','iAddress','iLat','iLng'].forEach(id=>{if($(id))$(id).value=''});toggleAdd(false);t.itinerary[curDate]=sortItems(t.itinerary[curDate]);save();
    if(address&&!found)alert('地址沒有找到座標，已儲存行程；你可以稍後編輯地址或手動填緯經度。')
  };
  ...
  function flightEditor(index=null){
    const t=trip(),f=index===null?{dir:'去程',date:curDate,flightNo:'',route:'',time:''}:t.flights[index];
    open(index===null?'新增航班':'編輯航班',field('uDir','航班類型','select',f.dir||'去程',['去程','回程','轉機','其他'])+field('uDate','日期','date',f.date||curDate)+field('uFlightNo','航班編號','text',f.flightNo||'')+`<div class="full">${field('uRoute','航線','text',f.route||'')}</div>`+`<div class="full">${field('uNote','時間／備註','text',f.time||'')}</div>`,()=>{const obj={dir:$('uDir').value,date:$('uDate').value,flightNo:$('uFlightNo').value.trim().toUpperCase(),route:$('uRoute').value.trim(),time:$('uNote').value.trim()};if(index===null)t.flights.push(obj);else t.flights[index]=obj;closeUniversalModal();save();setTimeout(decorateFlightNumbers,50)})
  }
  ...
  function decorateFlightNumbers(){
    const box=$('bookingInfo'),t=typeof trip==='function'?trip():null;if(!box||!t)return;
    const settings=[...box.querySelectorAll('.setting')];
    (t.flights||[]).forEach((f,i)=>{
      const card=settings[i];if(!card)return;
      card.querySelector('.flight-no-badge')?.remove();
      if(f.flightNo){const b=document.createElement('span');b.className='pill flight-no-badge';b.style.marginLeft='8px';b.textContent=f.flightNo;const bold=card.querySelector('b');bold?.insertAdjacentElement('afterend',b)}
    })
  }
